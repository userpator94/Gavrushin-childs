...
namespace Norms_physicalDev
{
    public partial class Form1 : Form
    {
        ...
        public void greetingForm()
        {
            ...
            greet.Show();
            //_pause(5000);
            var t = Task.Run(async delegate { await Task.Delay(TimeSpan.FromSeconds(2.6)); return 42; });
            t.Wait();
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            ////System.Diagnostics.Process.Start("childs_stats_sync.exe");
            ////toolTip1.SetToolTip(button2, "Синхронизировать с облаком")

            Form synchroForm = new synchro_form();
            synchroForm.StartPosition = FormStartPosition.CenterParent;
            synchroForm.Location = this.Location;
            synchroForm.Show(this);
            if (synchroForm.StartPosition == FormStartPosition.CenterParent)
            {
                var x = Location.X + (Width) - synchroForm.Width;
                var y = Location.Y;
                synchroForm.Location = new System.Drawing.Point(Math.Max(x, 0), Math.Max(y, 0));
            }                  
        }
        ...
        {
            string updfile = "https://drive.google.com/open?id=1To23evjQSw4mDM6KxSA_9KgR4xZNoNWHThk7YXgcmVI";
            string filename = "updfile.txt";
            try
            {
                using (WebClient client = new WebClient())
                {
                    client.DownloadFile(new Uri(updfile), filename);
                }

                string text = "";
                string[] textArr;
                using (StreamReader fs = new StreamReader(@"updfile.txt"))
                {
                    while (true)
                    {
                ...
                //var doc = XDocument.Parse(textArr[10]);
                string[] s1 = textArr[10].Split('"')[3].Split(' ');                
                string newVersion = s1[0];

                //foreach (var c in doc.Descendants("meta"))
                //{
                //    if (c.Attribute("property").Value.ToString().Equals("og:description"))
                //    {
                //        newVersion = c.Attribute("content").Value.Split(' ')[0];
                //        updateLink = c.Attribute("content").Value.Split(' ')[1];
                //        break;
                //    }                    
                //}
                if (!curVersion.Equals(newVersion))
                {
                    upd_Link.Visible = true;
                    updateLink = s1[1];
                }
            }
            catch(Exception e) { }
        ...
        private void upd_Link_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            upd_Link.LinkVisited = true;
            try
            {
